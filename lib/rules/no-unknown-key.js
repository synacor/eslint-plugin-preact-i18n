...
function checkForMissingKey({
  ...
  (0, _utils.getLangConfig)(config).forEach(({
    ...
    if (typeof pluralNode === 'undefined' && !(0, _utils.has)(translation, key, config.scopes)) {
      return context.report({
        node,
        severity: 2,
    ...
    let tranlationValue = (0, _utils.get)(translation, key, config.scopes);
