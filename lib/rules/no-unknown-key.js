...
const _require2 = require('../constants'),
      ...
      DEFAULT_WITH_TEXT_RE = _require2.DEFAULT_WITH_TEXT_RE;
...
module.exports = {
  ...
  create(context) {
    ...
    const ignoreFiles = config.ignoreFiles,
          ...
          withTextRegex = _config$withTextRegex === void 0 ? DEFAULT_WITH_TEXT_RE : _config$withTextRegex;
    ...
    return {
      ...
      JSXElement(node) {
        ...
        if (!nodeNameRE.test(nodeName) && !markupNodeNameRE.test(nodeName)) return; // Get the key value and pluralNode (optional) from the attributes of the JSX element
